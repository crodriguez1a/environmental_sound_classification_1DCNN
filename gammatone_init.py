...
def gammatone_impulse_response(samplerate_hz, length, center_freq_hz, phase_shift, p):
    ...
    length_in_seconds = (1.0 / samplerate_hz) * length
    t = np.linspace(1.0 / samplerate_hz, length_in_seconds, length)
    gammatone_ir = (
        a
        * np.power(t, p - 1)
        * np.exp(-2 * np.pi * b * t)
...
def generate_filters(num_filters, length, samplerate_hz, min_center_freq, p):
    ...
    for i in range(num_filters):
        filterbank[i, :] = gammatone_impulse_response(
            samplerate_hz, length, curr_center_freq, 0, p
        )
        curr_center_freq = erb_scale_2_freq_hz(
            freq_hz_2_erb_scale(curr_center_freq) + 1
        )
...
class GammatoneInit(tf.keras.initializers.Initializer):
    ...
    def __call__(self, shape, dtype=None):
        ...
        filters = generate_filters(
            shape[2], shape[0], self.sample_rate, self.min_center_freq, self.order
        )
